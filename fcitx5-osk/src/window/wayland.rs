...
impl WaylandWindowManager {
    ...
    fn open_window(&mut self, mut settings: WindowSettings) -> (Id, Task<WaylandMessage>) {
        ...
        if self.mode == WindowManagerMode::WaylandInputPanel && placement == Placement::Dock {
...
impl WindowManager for WaylandWindowManager {
    ...
    fn fetch_screen_info(&mut self) -> Task<Self::Message> {
        if self.mode == WindowManagerMode::WaylandInputPanel {
                ...
                s.size
                    ...
                    .filter(|_| {
                        s.placement == Placement::Dock
                            && self.mode != WindowManagerMode::WaylandInputPanel
                    })
            })
            .max()
            .unwrap_or(0);
