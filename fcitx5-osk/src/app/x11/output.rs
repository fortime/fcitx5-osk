...
use crate::{app::Message, dbus::server::ImPanelEvent};
...
impl OutputContext {
    ...
    fn send_reopen_if_opened_event(&self) {
        if self
            .tx
            .unbounded_send(Message::from(ImPanelEvent::ReopenIfOpened))
            .is_err()
        {
    ...
    fn add_or_update_output_info(&self, output_info: OutputInfo) {
        ...
        let selected_changed = guard.selected_output == Some(output_info.x11_output);
        let existed = guard
            .output_infos
            .iter()
            .position(|o| o.x11_output == output_info.x11_output);
        ...
        tracing::debug!(
            "Ouptut added: {}, selected changed: {}",
            output_info.x11_output,
            selected_changed
        );
        guard.output_infos.push(output_info);
        if selected_changed {
            self.send_reopen_if_opened_event();
        }
    }

    fn remove_output_info(&self, x11_output: u32) {
        tracing::debug!("Remove output: {x11_output}");
        let Some(mut guard) = self.state() else {
            tracing::warn!(
        ...
        let selected_changed = guard
            .selected_output
            .take_if(|s| *s == x11_output)
            .is_some();
        let existed = guard
            .output_infos
            .iter()
            .position(|o| o.x11_output == x11_output);
        tracing::debug!(
            "Output rmeoved: {}, selected changed: {}",
            existed.is_some(),
            selected_changed
        );
        if let Some(existed) = existed {
