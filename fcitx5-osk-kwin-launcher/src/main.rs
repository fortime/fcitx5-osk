use std::{
    collections::VecDeque, env, os::fd::{FromRawFd, OwnedFd}, process, sync::{
        ...
        match owner {
            ...
            Some(addr) => {
                ...
                if let Some(socket) = socket.take() {
                    ...
                    proxy
                        .change_mode(WindowManagerMode::WaylandInputPanel)
                        .await?;
                    proxy
...
async fn watch_kwin_virtual_keyboard(
    ...
    tokio::spawn({
        let fcitx5_osk_services = fcitx5_osk_services.clone();
        let kwin_services = kwin_services.clone();
        let exit = exit.clone();
        let f = async move |fcitx5_osk_services: Fcitx5OskServices,
                            kwin_services: KwinServices,
                            exit: Arc<AtomicBool>,
                            in_lockscreen: bool|
                    -> Result<()> {
            loop {
                ...
                if in_lockscreen && mode == Ok(WindowManagerMode::WaylandInputPanel) {
                    break;
                } else if !in_lockscreen && mode == Ok(WindowManagerMode::Normal) {
                    break;
            ...
            if in_lockscreen {
                ...
                while let Some(_) = stream.next().await {
                    ...
                    if visible != last_visible {
                        ...
                        fcitx5_osk_services.controller().change_visible(visible).await?;
                    }
        ...
        f(fcitx5_osk_services, kwin_services, exit, in_lockscreen)
    });
    let mut stream = fdo_services
        .screen_saver()
        .receive_active_changed()
        .await?;
...
async fn run(args: &Args) -> Result<()> {
    fcitx5_osk_common::log::init_log(&[], args.log_timestamp)?;
    ...
    tokio::select! {
        ...
        res = watch_kwin_virtual_keyboard(&services, &kwin_services, &fdo_services, lockscreen_active) => {
            if let Err(e) = res {
